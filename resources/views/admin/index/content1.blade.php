        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header bg-info">
                    <h3 class="text-center text-white">Edit Content-1 Info</h3>
                </div>
                @if (session('banner'))
                    <div class="alert alert-success">{{session('banner')}}</div>
                @endif
                <div class="card-body">
                    <form action="{{route('content1.update')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <label>Top Heading</label>
                        <input type="text" name="head" class="form-control" value="{{$content->head}}">
                    </div>
                    <div class="mb-3">
                        <label>Top Description</label>
                        <input type="text" name="head_desp" class="form-control" value="{{$content->head_desp}}">
                    </div>
                    <div class="mb-3">
                        <label>Title</label>
                    ...
                    <div class="mb-3">
                        ...
                        @endif
                        <div class="my-2">
                            <img width="100" id="blah" src="{{asset('upload/content1')}}/{{$content->image}}" alt="">
                        </div>
                    </div>
                    <div class="row">
